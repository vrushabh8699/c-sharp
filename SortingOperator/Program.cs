...
namespace SortingOperator
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var result4 = num.GroupBy(h => h.City);
            foreach(var item in result4)
            {
                Console.WriteLine($"City : {item.Key} Total Studant : {item.Count()}");
            }
            //Toookup-----------
            var patil1 = num.ToLookup(p => p.Name);
            foreach (var item in patil1)
            {
                Console.WriteLine($"total studant : {item.Count()} city : {item.Key} ");
            }
